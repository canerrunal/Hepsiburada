        ...
        try:
            ...
            for root in roots:
                ...
                for category in descendants:
                    if len(unique) >= args.max_products:
                        break
                    stagnant = 0
                    for page_no in range(1, args.max_pages_per_category + 1):
                        ...
                        try:
                                ...
                                for product in norm_product(item, category.get("id"), pos, page_no, None):
                                    ...
                                    if key not in unique:
                                        ...
                                        product.update({
                                            ...
                                            "category_path": category.get("full_path") or category.get("path"),
                            ...
                            blocked = str(error)[:120]
                            break
                    if blocked:
                        break
                rankings_out.extend(membership)
                status = "PASS" if len(unique) >= args.max_products else "INSUFFICIENT_SOURCE"
                summary["roots"].append({"root_id": root.get("id"), "root_name": root.get("name"),
                                         "category_count": len(descendants), "unique_product_count": len(unique),
                                         "target": args.max_products, "status": status, "error": blocked})
        finally:
